...
public abstract class Library extends OpMode {
	...
	public void hardwareInit() {
		...
		// Safety Check: run through the list of voltage sensors; if any of them are
		// below the minimum voltage, exit.
		/*
		 * for (VoltageSensor voltageSensor : hardwareMap.voltageSensor) { voltSensor =
		 * voltageSensor; if (voltageSensor.getVoltage() < WARNING_BATTERY_VOLTAGE)
		 * telemetry.addData("WARNING: ", "BATTERY LOW"); if (voltageSensor.getVoltage()
		 * < REPLACE_BATTERY_VOLTAGE) telemetry.addData("WARNING: ",
		 * "BATTERY VERY LOW; REPLACE IMMEDIATELY"); }
		 */
	}
	...
	/*
	 * public void battery(){ voltSensor.getVoltage();
	 * telemetry.addData("Initial Battery: ", voltSensor); try { if (voltSensor !=
	 * voltSensor){ telemetry.addData("Battery Level: ", voltSensor); } }
	 * catch(Exception ex) {} }
	 */

	/*
	 * drive is the central movement and robot handling method of the code It takes
	 * in the linear (forward) component, the rotational (turning) component, and
	 * the side(skating) component. It creates arrays for each of the components,
	 * and then adds them all together. After finding the highest value, it makes
	 * sure than everything is below 1. If any values are above 1, it divides all
	 * the sums by 1, else it divides by the highest value. It then sends the values
	 * from the modified sums array to the actual motors, with the code having
	 * numbers attached to them to account for proper rotation.
	 */
	public static void drive(float l, float r, float s, float intake) {
		 ...
		 * 
		 * @param: l, linear component, r, rotational component, and s, horizontal
		 * component
		 */
	...
	/*
	 * Parameter(s): float cM, String mode "turn", float degree(optional), float
	 * directionSideways (optional) cM: distance you want it to travel in
	 * centimeters mode: either forwards, sideways, or turning (this will control
	 * what the thing does) degree: if you are turning fill this in, otherwise it
	 * will go straight, this will determine what angle you want the robot to turn
	 * directionSideways: also a degree, no radians PAUL, it determines which
	 * diagonal path the robot takes if we want to be extra like that
	 */
	...
	public static void turnDegrees(int degrees) {
		...
		// It calculates a "circle" that starts in the center of the robot and hits all
		// 4 wheels
		// The arc length for the given degree is the CM the robot turnFor()
		float radius = Math.sqrt((wheelToWheelWidth / 2.0f) * (wheelToWheelWidth / 2.0f)
				+ (wheelToWheelLength / 2.0f) * (wheelToWheelLength / 2.0f));
	...
	public static int encodersLagQuestionMark(DcMotor one, DcMotor two, DcMotor three, DcMotor four) {
		return (one.getCurrentPosition() + two.getCurrentPosition() + three.getCurrentPosition()
				+ four.getCurrentPosition()) / 4;
	}

	// This method allows the robot to turn a certain number of degrees using
	// encoders
	...
	/*
	 * hardware notes: 2 motors for lift - need to be in sync, have intervals that
	 * it can go to manually (button press, may require encoders) 1 servo for block
	 * grabber - likely continuous (button press)
	 */
	 ...
	 * 
	 * @param patternNumber Integer [1,100] that represents desired LED pattern or
	 *                      color
	 */
	/*
	 * public static void setBlinkinPattern(int patternNumber) { //map patternNumber
	 * value from [1,100] => [0.2525, 0.7475] //formula: output = output_start +
	 * ((output_end - output_start) / (input_end - input_start)) * (input -
	 * input_start) double output = 0.2525 + ((0.7475 - 0.2525) / (100 - 1)) *
	 * (patternNumber - 1);
	 * 
	 * //send mapped value to "servo" (Blinkin) blinkin.setPower(output); }
	 */
