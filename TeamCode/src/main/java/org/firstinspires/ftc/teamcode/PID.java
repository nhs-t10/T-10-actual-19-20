...
public class PID {
	...
	public void updateMoving() {
		...
		// code to do control movemment of the robot, in order to get to an "ideal
		// state"
	}
	...
	// 2) Differential power between left and right. You calculate the overall
	// forward
	// power by checking your encoder against its target. So if the target is far
	// away,
	// the overall forward power is large. If you are almost there, the overall
	// forward power
	// would be small. Then the differential power is controlled by the gyro. If the
	// robot is
	// deviated from the center line in a big way (i.e. the error is large), the
	// differential
	// power is big. If you are right on straight, the differential power is zero.
	// With these
	...
	/*
	 * driveError = distanceTarget - currentEncoder; turnError = angleTarget -
	 * currentGyroAngle; drivePower = driveKp * driveError; turnPower =
	 * turnKp*turnError; leftPower = drivePower + turnPower; rightPower =
	 * driverPower - turnPower;
	 */
